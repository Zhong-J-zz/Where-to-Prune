...
class PRUNE_RESNET_CNN(nn.Module):
    ...
    def __init__(self, net, to_add_conv, add_tuple):
        ...
        for in_chanels, out_channels in add_tuple:
            basic_block.append(BasicConv2d(
                in_chanels, out_channels, kernel_size=1, stride=1, padding=0))
            ...
            if isinstance(x, torch.nn.ReLU):
                ...
                if num_conv - 1 in self.to_add_conv:
                    ...
                    if add_layer.size()[1] != y.size()[1]:
                        add_layer = self.basic_conv2d[
                            basic_conv_index](add_layer)
                        basic_conv_index += 1
...
def make_layers(network, n_pool=5):
    ...
    for x in network:
        if 'C:' in x:
            ...
            if units == 0:
                units = int(in_channels * 32 * 32 /
                            pow(2, n_pool) / pow(2, n_pool))
            classification += [nn.Linear(units, int(x.split(':')[1])), nn.BatchNorm1d(int(
                x.split(':')[1]), eps=0.001, momentum=0.1, affine=True), nn.ReLU(inplace=False)]
            units = int(x.split(':')[1])
    if units == 0:
