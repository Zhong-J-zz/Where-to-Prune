...
vgg19 = ['C:64', 'C:64', 'P', 'C:128', 'C:128', 'P', 'C:256', 'C:256', 'C:256', 'C:256', 'P',
         'C:512', 'C:512', 'C:512', 'C:512', 'P', 'C:512', 'C:512', 'C:512', 'C:512', 'P', 'S:10']
prune_percent = 0.8
network_dict = []
this_network = vgg19
...
def parse_arguments(argv):
    ...
    parser.add_argument('--pretrained_model', type=str,
                        default='/1down_params_lstm_prune.pkl')
    ...
    parser.add_argument('--history_network_npy', type=str,
                        default='/network_prune.npy')
    parser.add_argument('--use_history_network',
                        action='store_true', default=False)
    parser.add_argument('--lr', type=float, default=0.001)
    parser.add_argument('--train_model', type=str,
                        choices=['lstm', 'lstm_fullcov', 'lstm_prune'], default='lstm_prune')
    parser.add_argument('--save_lstm_path', type=str,
                        default='/lstm_prune.pkl')
    parser.add_argument('--save_history_npy', type=str,
                        default='/network_prune.npy')
...
def gen_trainset(network_dict, batch_lstm):
    if(len(network_dict) < batch_lstm):
        ...
        trainset = [[network_dict[i][1], network_dict[i][2]]
                    for i in range(batch_lstm)]
        return trainset
        ...
        if this_network[i] != 'P' and 'S' not in this_network[i]:
            ...
            if top_dec[index] == 1:
                if this_network[i].split(':')[0] == 'C':
                    c = int(
                        int(network[i + pos - 1].split(':')[1]) * prune_percent)
...
def calculate_loss(reward, zero_reward, dec_scores, dec):
    ...
    reward_array = autograd.Variable(torch.from_numpy(np.array(
        [[reward_array[i], reward_array[i]]for i in range(len(reward_array))]))).float().cuda()
    mask_dec = torch.cat([x != x.max() for x in dec]).resize(
        dec.size()[0], dec.size()[1]).float()
    mask_dec1 = torch.cat([x == x.max() for x in dec]).resize(
        dec.size()[0], dec.size()[1]).float()
...
def change_network(net, target, del_filters, which_nn):
    for layer_index, filter_index, f in target:
        if which_nn == 'features':
            if f == -1:
                net = prunepackage.prune_per_conv_layer(
                    net, layer_index, filter_index)
    return net
...
def pruning(index, network, del_filters, cnn_epoch_size, lr, use_teacher, trainloader, testloader, finalloader, model_teacher):
    ...
    for layer, module in enumerate(net.features):
        if isinstance(module, nn.Conv2d):
            if index > 0 and del_filters[index - 1] > 1:
                ...
                filter_prune[
                    layers_prune[-1]] = get_filter_prune(filter_weights, del_filters[index - 1], False)
                pre_bn_weights = 1
    ...
    if pre_bn_weights != 1:
        ...
        linear_weights = [0.9 * linear_weights[i]
                          for i in range(len(linear_weights))]
        if del_filters[index - 1] > 1:
            filter_prune[
                layers_prune[-1]] = get_filter_prune(linear_weights, del_filters[index - 1], False)
    ...
    reward, acc, net = finetune(net, network, cnn_epoch_size, lr,
                                use_teacher, trainloader, testloader, finalloader, model_teacher)
        ...
        if 'P' not in x and 'S' not in x:
            if top_out == 'WIDE':
                if top_dec[index] == 0:
                    ...
                    del_filters.append(
                        int(x.split(':')[1]) - int(int(x.split(':')[1]) * prune_percent))
                    zero_reward.append(min(
                        abs(int(x.split(':')[1]) - int(int(x.split(':')[1]) * prune_percent)) - 0.5, 1))
            index += 1
    if zero_reward == []:
...
def finetune(net, network, cnn_epoch_size, lr, use_teacher, trainloader, testloader, finalloader, model_teacher):
    ...
    for epoch in range(cnn_epoch_size):
        ...
        for i, data in enumerate(trainloader, 0):
            inputs, labels = data
            inputs, labels = autograd.Variable(
                inputs.cuda(), requires_grad=True), autograd.Variable(labels.cuda())
            optimizer.zero_grad()
            outputs = net(inputs)
...
def train(args):
    ...
    if args.use_teacher:
        ...
        model_teacher.cuda()
        model_teacher.eval()
    trainloader, testloader, finalloader = data_process(
        args.data_dir, args.cnn_batch_size)
    ...
    for epoch in range(args.lstm_epoch_size):
        ...
        for index, data in enumerate(train_set):
            ...
            if args.train_model == 'lstm_prune':
                ...
                del_filters, zero_reward = get_del_filters(
                    dec_scores, this_network)
            if (this_network, new_network) in [(network_dict[i][0], network_dict[i][1]) for i in range(len(network_dict))]:
                this_reward = network_dict[[(network_dict[i][0], network_dict[i][1]) for i in range(
                    len(network_dict))].index((this_network, new_network))][2]
                acc = network_dict[[(network_dict[i][0], network_dict[i][1]) for i in range(
                    len(network_dict))].index((this_network, new_network))][3]
                continue_networks += 1
                ...
                netcnn, this_reward, acc = pruning(index, new_network, del_filters, args.cnn_epoch_size,
                                                   args.lr, args.use_teacher, trainloader, testloader, finalloader, model_teacher)
                if this_reward <= best_reward:
                    continue_networks += 1
                else:
                    continue_networks = 0
                network_dict.append(
                    [this_network, new_network, this_reward, acc])
                network_num += 1
            ...
            if args.train_model == 'lstm_prune':
                loss_back = calculate_loss(
                    reward, zero_reward, dec_scores, dec)
            print('epoch: %d no. of network: %d this_reward: %f,best_reward:%f\n  loss:' % (
                epoch, network_num, this_reward, best_reward), loss_back)
            loss_back.backward()
            optimizer_lstm.step()
            network_dict.sort(key=lambda x: x[2], reverse=True)
        ...
        for i in range(lenth):
            if(id_list[i] + 1 > len(old_net)):
                shutil.copy(
                    '1model' + str(id_list[i] - len(old_net)) + '.pkl', '2model' + str(i) + '.pkl')
            else:
                shutil.copy(
                    'model' + str(id_list[i]) + '.pkl', '2model' + str(i) + '.pkl')
        for i in range(lenth):
